...
class Generator(nn.Module):
    ...
    '''
    def __init__(self, in_channels=32, out_channels=3, up_sizes= [4,2,2,4]):
        super().__init__()
        self.fc = FF(1024, 512)
        self.conv1 = up_block(in_channels, 64)
        self.conv2 = up_block( 64, 64, kernel_size = 4, stride = up_sizes[0])
        self.conv3 = up_block(64, 64)
        self.conv4 = up_block(64, 64, kernel_size = 2, stride=up_sizes[1])
        self.conv5 = up_block(64, 64)
        self.conv6 = up_block(64, 32, kernel_size = 2, stride=up_sizes[2])
        self.conv7 = up_block(32,3,act='sigmoid')
...
class Discriminator(nn.Module):
    ...
    def __init__(self, in_channels=3):
        super().__init__()
        self.conv1 = down_block(in_channels,16)
        self.conv2 =  down_block(16,32)
        self.conv3 = down_block(32,64)
...
def gen_loss(fake_outs):
    loss = torch.mean(0.5 * (fake_outs - 1) ** 2)

    return loss


def disc_loss(real_outs, fake_outs):
    d_loss = 0.5 * (real_outs - 1) ** 2
    g_loss = 0.5 * fake_outs ** 2

    return torch.mean(d_loss + g_loss)
